...
const PrescriptionItemSchema = zod_1.z.object({
    medId: zod_1.z.string(),
    doseAmount: zod_1.z.number(),
    doseUnit: zod_1.z.string(),
    route: zod_1.z.string(),
    frequency: zod_1.z.string(),
    durationDays: zod_1.z.number(),
    withholdingHours: zod_1.z.number().optional(),
});
...
const ChargeSchema = zod_1.z.object({
    ...
    category: zod_1.z.enum(['board', 'training', 'med', 'misc']).optional(),
});
const InvoiceSchema = zod_1.z.object({
    horseId: zod_1.z.string().optional(),
    periodFrom: zod_1.z.string().datetime(),
    periodTo: zod_1.z.string().datetime(),
});
const InvoiceUpdateSchema = zod_1.z.object({
    status: zod_1.z.enum(['draft', 'sent', 'paid']).optional(),
    externalPaymentUrl: zod_1.z.string().url().optional(),
});
const HorseImportSchema = zod_1.z.object({
    name: zod_1.z.string(),
 ...
 *         description: Internal server error
 */
app.post('/vaccinations', (0, auth_1.allowRoles)('MANAGER', 'VET'), async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.post('/administrations', (0, auth_1.allowRoles)('MANAGER', 'VET'), async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.post('/prescriptions', (0, auth_1.allowRoles)('MANAGER', 'VET'), async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.get('/horses/:horseId/prescriptions', auth_1.requireAuth, async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.post('/billing/charges', (0, auth_1.allowRoles)('MANAGER', 'BILLING'), async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.get('/billing/charges', auth_1.requireAuth, async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.post('/billing/invoices', (0, auth_1.allowRoles)('MANAGER', 'BILLING'), async (req, res) => {
    try {
 ...
 *         description: Internal server error
 */
app.get('/billing/invoices/:id', auth_1.requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 *         description: Internal server error
 */
app.get('/billing/invoices/:id/pdf', auth_1.requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const invoice = await prisma.invoice.findUnique({
            where: { id },
 ...
 *         description: Internal server error
 */
app.patch('/billing/invoices/:id', (0, auth_1.allowRoles)('MANAGER', 'BILLING'), async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/export/vaccinations.csv', async (req, res) => {
    try {
        ...
        if (from || to) {
            where.doseDatetime = {};
            if (from)
                where.doseDatetime.gte = new Date(from);
            if (to)
                where.doseDatetime.lte = new Date(to);
        }
        ...
        const csvRows = vaccinations.map(v => `"${v.horse.name}","${v.product}","${v.batchNo || ''}","${v.expiryDate?.toISOString().split('T')[0] || ''}","${v.doseDatetime.toISOString().split('T')[0]}","${v.site || ''}","${v.vetId || ''}"`).join('\n');
...
app.get('/export/administrations.csv', async (req, res) => {
    try {
        ...
        if (from || to) {
            where.datetimeLocal = {};
            if (from)
                where.datetimeLocal.gte = new Date(from);
            if (to)
                where.datetimeLocal.lte = new Date(to);
        }
        ...
        const csvRows = administrations.map(a => `"${a.horse.name}","${a.rxItem.med.generic}","${a.actualDose || ''}","${a.route || ''}","${a.batchNo || ''}","${a.lotNo || ''}","${a.expiryDate?.toISOString().split('T')[0] || ''}","${a.barcodeValue || ''}","${a.notes || ''}","${a.datetimeLocal.toISOString().split('T')[0]}"`).join('\n');
        const csv = csvHeaders + csvRows;
        res.setHeader('Content-Type', 'text/csv');
